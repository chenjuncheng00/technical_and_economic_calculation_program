...
Public Class 设置长期贷款计算方式
    ...
    Private Sub 确定计算方式_Click(sender As Object, e As EventArgs) Handles 开始计算.Click
        ...
        If XZ = vbOK Then
            ...
            '————————————————————————————————————————————————————————————————————————————————————————————————————————————
            '计算长期贷款
            Call 长期贷款相关计算(ExcelApp, sdsl_model, zbj_model, xlfl_cg_model, xlfl_qt_model, kcje_xlf_model, hscz, clfl_qtfl_model, kcje_clf_qtf_model, ldzj_model, kcje_ldzj_model, bxf_model, kcje_bxf_model)
            '————————————————————————————————————————————————————————————————————————————————————————————————————————————
            '显示
